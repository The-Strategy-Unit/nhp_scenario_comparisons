...
mod_principal_change_factor_effects_summarised_ndg1 <- function(data, measure, include_baseline) {
  data <- data |>
    dplyr::filter(
  ...
  cfs <- data |>
    ...
    dplyr::mutate(
      ...
      colour = dplyr::case_when(
        .data$change_factor == "Baseline" ~ "#f9bf07",
        .data$value >= 0 ~ "#f9bf07",
        TRUE ~ "#f9bf07"
      ),
...
mod_principal_change_factor_effects_summarised_ndg2 <- function(data, measure, include_baseline) {
  data <- data |>
    dplyr::filter(
      .data$measure == .env$measure,
      include_baseline | .data$change_factor != "baseline",
