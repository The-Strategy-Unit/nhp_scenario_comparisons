  ...
  data |>
    ...
    dplyr::mutate(
      ...
      "activity_type" = dplyr::case_when(
        ...
        stringr::str_detect(.data$pod_name, "Admission") ~ glue::glue(
          "{.data$activity_type} Admissions"
        ),
        stringr::str_detect(.data$pod_name, "Bed Days") ~ glue::glue(
          "{.data$activity_type} Bed Days"
        ),
